...
void String::goTo(char a) {
    int index = 0;
    while (str[index] != a && str[index] != TERMINATE_SYMBOL) {
        index++;
    }
    if (str[index] != TERMINATE_SYMBOL) {
        index++;
    }
...
void String::goTo(const char *target) {
    ...
    while (true) {
        ...
        for (int i = 1; i < length; ++i) {
            if (target[i] != str[i - 1]) {
                isTarget = false;
                break; //it is not the word that we search for
            }
        }
        if (isTarget) {
            index += (length - 1);
...
void String::cut(unsigned int index) {
    size -= index;
...
String String::takeTo(char symbol) {
    char nextWord[BUFFER_SIZE];
    int index = 0;
    while (str[index] != symbol && str[index] != TERMINATE_SYMBOL) {
...
void String::move(unsigned int n) {
    if (n >= size) {
        size = 0;
